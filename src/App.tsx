...
const ALL_KEYS = [WEEKLY_GOAL_KEY, ...DAYS];
...
export default function App() {
  ...
  const renderCard = (key: string, isWeeklyGoal: boolean = false) => {
    ...
    return (
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className={`text-lg font-semibold flex items-center gap-2 ${isWeeklyGoal ? 'text-orange-800' : 'text-stone-800'}`}>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <select
                ...
                className={`appearance-none text-xs font-medium bg-transparent border border-transparent hover:border-stone-200 rounded px-2 py-1 pr-5 outline-none cursor-pointer transition-colors ${isWeeklyGoal ? 'text-orange-600 hover:bg-orange-100/50' : 'text-stone-500 hover:bg-stone-50'}`}
                title="並び替え"
              >
              ...
              <ArrowUpDown size={12} className={`absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none ${isWeeklyGoal ? 'text-orange-400' : 'text-stone-400'}`} />
            </div>
            <span className={`text-xs font-medium ${isWeeklyGoal ? 'text-orange-500' : 'text-stone-400'}`}>{progress}%</span>
          </div>
        </div>
        
        {/* Progress Bar */}
        <div className={`w-full rounded-full h-1.5 mb-5 overflow-hidden ${isWeeklyGoal ? 'bg-orange-100' : 'bg-stone-100'}`}>
          <div 
        ...
        {/* Task List */}
        <div className={`flex-1 overflow-y-auto mb-4 space-y-2 ${isWeeklyGoal ? 'min-h-[200px]' : 'min-h-[150px]'}`}>
        ...
        <div className="mt-auto pt-3 border-t border-stone-100">
          <form 
            onSubmit={(e) => { e.preventDefault(); addTask(key); }}
            className="flex flex-col gap-2"
          >
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setNewTaskPriority(prev => ({ ...prev, [key]: cyclePriority(prev[key] || 'low') }))}
                className={`p-2 rounded-lg transition-colors hover:bg-stone-100 flex-shrink-0 ${getPriorityColor(newTaskPriority[key] || 'low')}`}
                title="優先度を設定 (低・中・高)"
              >
                <Flag size={16} className={newTaskPriority[key] === 'high' ? 'fill-current' : ''} />
              </button>
              <button
                type="button"
                onClick={() => setNewTaskRecurrence(prev => ({ ...prev, [key]: cycleRecurrence(prev[key]) }))}
                className={`p-2 rounded-lg transition-colors flex items-center flex-shrink-0 ${getRecurrenceColor(newTaskRecurrence[key])}`}
                title={`繰り返し: ${getRecurrenceLabel(newTaskRecurrence[key])}`}
              >
                <Repeat size={16} />
                {newTaskRecurrence[key] && newTaskRecurrence[key] !== 'none' && (
                  <span className="text-[10px] font-bold ml-0.5 leading-none">
                    {newTaskRecurrence[key] === 'daily' ? '日' : newTaskRecurrence[key] === 'weekly' ? '週' : '月'}
                  </span>
                )}
              </button>
              <input
                ...
                className="flex-1 bg-stone-50 border-transparent focus:bg-white focus:border-stone-300 focus:ring-0 text-sm rounded-lg px-3 py-2 transition-all outline-none border min-w-0"
              />
              <button 
                type="submit"
                disabled={!newTaskText[key]?.trim()}
                className="p-2 text-stone-400 hover:text-stone-700 disabled:opacity-50 transition-colors rounded-lg hover:bg-stone-100 flex-shrink-0"
              >
                <Plus size={18} />
              </button>
            </div>
            {newTaskText[key]?.trim() && (
              <div className="flex items-center pl-10 pr-10 animate-in fade-in slide-in-from-top-2 duration-200 gap-2 flex-wrap">
                <div className="flex items-center gap-1.5 bg-stone-50 border border-stone-200 rounded-md px-2 py-1">
                  ...
                  <input
                    type="date"
                    value={newTaskDueDate[key] || ''}
                    onChange={(e) => setNewTaskDueDate(prev => ({ ...prev, [key]: e.target.value }))}
        ...
        {activeTab === 'tasks' ? (
          <div className="flex flex-col lg:flex-row gap-6 items-start animate-in fade-in duration-500">
            ...
            <div className="w-full lg:w-80 xl:w-96 flex-shrink-0">
              {renderCard(WEEKLY_GOAL_KEY, true)}
            </div>
            
            {/* Days Grid */}
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {DAYS.map(day => renderCard(day, false))}
            </div>
          </div>
        ) : (
          renderHistory()
